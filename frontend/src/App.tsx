...
function App() {
  ...
  const [detailRestaurant, setDetailRestaurant] =
    useState<ShopDetailSupplement | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      >
        ...
        {genresError && (
          <ErrorMessage message={genresError} onClose={clearGenresError} />
        )}
      ...
      {hasPagination && searchResult && (
        <div
          ...
          className="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/70 px-4 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-3 backdrop-blur-sm"
        >
          <div className="container flex gap-4 items-center justify-between">
            <Pagination
              currentPage={currentPage}
              totalCount={searchResult.results_available}
              count={ITEMS_PER_PAGE}
              onPageChange={handlePageChange}
              disabled={isLoading}
              start={searchResult.results_start}
              available={searchResult.results_available}
            />
            <ScrollToTopButton />
          </div>
        </div>
